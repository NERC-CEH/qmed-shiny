...
server <- function(input, output){
    ...
    inputPoint <- reactiveValues(long = 70,
                                 lat = 17)
    ...
    observeEvent(input$enLocation, {  
      ...
      leafletProxy('map') %>%
        ...
        if (!is.null(input$easting) & !is.null(input$northing)) {
            inputPoint$long <- as.numeric(input$easting)
            inputPoint$lat <- as.numeric(input$northing) 
            errorPoint$er <- F
            
            if (inputPoint$long > 90 || inputPoint$long < 40 ||
                inputPoint$lat < 8 || inputPoint$lat > 25) {
                errorPoint$er <- T
            }
    ...
    observeEvent(input$map_click, {
      ...
      inputPoint$lat <- click$lat
      inputPoint$long <- click$lng
    })
      
    
    observeEvent(inputPoint, {
      leafletProxy('map') %>%
          clearGroup('markers') %>%
          clearGroup('catchments')
      
      mapPoint$mapP <- SpatialPoints(
        data.frame(x=inputPoint$long,
                   y=inputPoint$lat))
      ...
      leafletProxy('map') %>%
        addMarkers(lng=inputPoint$long, lat=inputPoint$lat, group='markers')
      if (!is.null(location$zCode)) {
